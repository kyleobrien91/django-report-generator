...
class GetFieldsMixin(object):
    def get_fields(self, model_class, field_name='', path='', path_verbose=''):
        ...
        if field_name != '':
            ...
            if is_m2m and hasattr(field[0], 'm2m_reverse_field_name'):
            ...
            if is_direct: # Direct field
                new_model = field_object.related.parent_model
                path_verbose = new_model.__name__.lower()
            else: # Indirect related field
                new_model = field_object.model
                path_verbose = new_model.__name__.lower()

            fields = get_direct_fields_from_model(new_model)

            if hasattr(new_model, 'ReportBuilder'):
                report_builder_class = new_model.ReportBuilder

                if hasattr(report_builder_class, 'report_builder_exclude_fields'):
                    good_fields = []
                    for field in fields:
                        if not field.name in report_builder_class.report_builder_exclude_fields:
                            good_fields += [field]
                    fields = good_fields
    ...
    def get_related_fields(self, model_class, field_name, path="", path_verbose="", exclude = []):
        ...
        new_fields = get_relation_fields_from_model(new_model, exclude)

        model_ct = ContentType.objects.get_for_model(new_model)

        if path_verbose:
            path_verbose += "::"
        path_verbose += field_object.name
